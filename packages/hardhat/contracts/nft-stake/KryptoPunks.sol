...
contract KryptoPunks is ERC721AQueryable, Ownable {
  ...
  function mint(uint256 _mintAmount) external payable {
    ...
    if (_mintAmount > maxMintAmountPerTx) revert KryptoPunks__MaxMintAmountExceeded();
    uint256 supply = totalSupply();
    if (supply + _mintAmount > maxSupply) revert KryptoPunks__NftSupplyLimitExceeded();

    if (msg.sender != owner()) {
      if (msg.value < cost * _mintAmount) revert KryptoPunks__InsufficientFunds();
    }
  ...
  function setMaxMintAmountPerTx(uint256 _newmaxMintAmount) external payable onlyOwner {
